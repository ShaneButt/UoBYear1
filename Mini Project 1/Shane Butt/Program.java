...
class Program
{
  ...
  public void indentJava()
  {
      String previousJava = "";
      int block = 0;
      
      String openBracketsREGEX = "[\\{]$";
      String closeBracketsREGEX = "[\\}]$";
      
      Pattern openBracketsPattern = Pattern.compile(openBracketsREGEX);
      Pattern closeBracketsPattern = Pattern.compile(closeBracketsREGEX);
      
      for (JavaLine line : stored)
      {
          String javaLine = line.getJavaLine();
          Matcher openBracketsMatcher = openBracketsPattern.matcher(javaLine);
          Matcher previousOpenBracketsMatcher = openBracketsPattern.matcher(previousJava);
          Matcher closeBracketsMatcher = closeBracketsPattern.matcher(javaLine);
          Matcher previousCloseBracketsMatcher = closeBracketsPattern.matcher(previousJava);
          
          if (previousOpenBracketsMatcher.find() )
          {
              block++;
              line.setBlockIndent(block);
          } 
          else if (closeBracketsMatcher.find() && !javaLine.contains("{"))
          {
              block--;
              line.setBlockIndent(block);
          }
  ...
  public int findMaxJavaLineLength()
  {
    ...
    for ( JavaLine line : stored ) {
      ...
      int lineBlock  = line.getBlockIndent();
      if ( lineLength > max ) { max = lineLength + (lineBlock * 2); }
    }
    return max;
  }
